...
namespace com.mirle.ibg3k0.sc.Data.ValueDefMapAction
{
    ...
    public class EQTcpIpMapAction : ValueDefMapActionBase, IDynamicMetaObjectProvider
    {
        ...
        protected void str134_ReceiveProcess(object sender, TcpIpEventArgs e)
        {
            ...
            if(!IsNeedProcess134Event(e))
            {
                return;
            }
            scApp.VehicleBLL.setAndPublishPositionReportInfo2Redis(eqpt.VEHICLE_ID, recive_str);
        }
        object PositionRefresh_Sync = new object();
        int PrePositionSeqNum = 0;
        private bool IsNeedProcess134Event(TcpIpEventArgs e)
        {
            bool need_process_position = true;
            int pre_position_seq_num = 0;
            int current_seq_num = 0;
            lock (PositionRefresh_Sync)
            {
                current_seq_num = e.iSeqNum;
                pre_position_seq_num = PrePositionSeqNum;
                //bool need_process_position = true;
                //lock (eqpt.PositionRefresh_Sync)
                //{
                need_process_position = checkPositionSeqNum(current_seq_num, pre_position_seq_num);
                PrePositionSeqNum = current_seq_num;
            }
